...
clear all
use *netto psample letztbef using ~/data/soep/soep.v32-gen/pre/ppfad.dta
...
local row=1
qui foreach wv in a b c d e f g h i j k l m n o p q r s t u v w x y z ba bb bc bd be bf {
   forvalues i=1/`cols' {
...
local colnm "A B C D E F G H I J K L1 L2 L3 M1 M2"
...
#d;
graph bar (first) A B C D E F G H I J K L1 L2 L3 M1 M2, over(year, label(angle(ninety))) stack 
   ...
   bar(16, lcolor(black) lpattern(solid))
	ylabel(, angle(zero)) 
